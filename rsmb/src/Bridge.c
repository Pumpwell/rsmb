...
void Bridge_stop(Bridges* bridge)
{
	...
	if (bridge->connections != NULL)
	{
		...
		{
			BridgeConnections* conn = (BridgeConnections*)curelement->content;

			conn->state = CONNECTION_STOPPING;
			conn->start_type = START_MANUAL; /* make sure the connection is not restarted automatically */
			conn->stop_was_manual = 1;
		}
	}
	FUNC_EXIT;
}
...
void Bridge_newConnection(BridgeConnections* bc, Clients** pclient)
{
...
#if defined(MQTTS)
			&& bc->protocol != PROTOCOL_MQTTS_MULTICAST
#endif
			)
	{
	...
	else if (bc->protocol == PROTOCOL_MQTTS_MULTICAST)
	{
		*pclient = MQTTSProtocol_create_multicast((char*)(bc->addresses->first->content),
				clientid, bc->loopback);
...
void Bridge_restartConnection(BridgeConnections* bc, Clients** pclient)
{
	...
	FUNC_ENTRY;
	time(&(now));
	if (bc->try_private && (bc->last_connect_result == CONNACK_NONE_RECEIVED || bc->last_connect_result == CONNACK_UNACCEPTABLE_PROTOCOL_VERSION) && (*pclient)->noLocal)
	{
		char* addr;
		(*pclient)->noLocal = 0;
		if (bc->round_robin == 0 && pclient == &(bc->backup))
...
void Bridge_processPartConnection(BridgeConnections* bc, Clients** pclient)
{
	...
	else
	{
		...
		{
			int connect_timeout = (bc->keepalive_interval == 0) ? 30 : bc->keepalive_interval;
			time_t now;

			time(&(now));
			/* the time allowed before a connect must complete now defaults to the keepalive interval */
			if (difftime(now, (*pclient)->lastContact) > connect_timeout)
			{
	...
	else
	{
		...
		while (ListNextElement(bc->topics, &elem))
		{
			...
			if (!curtopic->subscribed)
			{
				...
				{
					char* fulltopic = Bridge_addPrefix(curtopic->pattern, curtopic->localPrefix, &len);
					int qos = (client->protocol == PROTOCOL_MQTTS_MULTICAST) ? 0 : 2;
					if (SubscriptionEngines_subscribe(subsengine, client->clientID,				 /* local subscription */
							fulltopic, qos, 1, (client->cleansession == 0), curtopic->priority)) /* this is noLocal (and keep retained flags) */
						/* retained messages only sent if the subscription was new */
						MQTTProtocol_processRetaineds(client, fulltopic, qos, curtopic->priority);
 ...
 */
int Bridge_handleConnacks(void* pack, int sock, Clients* client)
{
...
BridgeTopics* Bridge_stripAndReplace(BridgeConnections* bc, Publish* publish, int outbound)
{
	...
	while (ListNextElement(bc->topics, &elem))
	{
		...
		if (Topics_matches(source, Topics_hasWildcards(source), publish->topic))
		{
			if (match)
			{
